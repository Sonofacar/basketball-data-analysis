...
dataset$Player_ID <- as_factor(dataset$Player_ID)
```

```
Names <- dbReadTable(conn, "player_info") %>%
	as_tibble() %>%
	select(Player_ID, Name)

player_season_totals %>%
	inner_join(Names, by = c('Player_ID' = 'Player_ID')) %>%
	arrange(desc(O_W_shares)) %>%
	View()
```
