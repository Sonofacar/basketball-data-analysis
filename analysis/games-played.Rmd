...
team_and_opponents <- team_games %>%
	...
	mutate(Poss_team = possessions(Two_Attempts_team + Three_Attempts_team,
				       Freethrow_Attempts_team,
				       Offensive_Rebounds_team,
				       Turnovers_team),
	       Poss_opponent = possessions(Two_Attempts_opponent + Three_Attempts_opponent,
				           Freethrow_Attempts_opponent,
				           Offensive_Rebounds_opponent,
				           Turnovers_opponent),
	       Poss_avg = possessions_avg(Twos_team + Threes_team,
...
player_season_totals <- dbReadTable(conn, 'player_games') %>%
	...
	summarize(MP = sum(Seconds) / 60,
		  ...
		  team_Avg_Poss = mean(Poss_team),
		  team_Avg_Pace = mean(Pace_team),
		  ...
		  opp_Avg_Poss = mean(Poss_opponent),
		  opp_Avg_Pace = mean(Pace_opponent),
		  Poss = sum(Poss_avg)
	) %>%
